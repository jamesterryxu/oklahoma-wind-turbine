...
def process_tsv(tsv_file):
    df = pd.read_csv(tsv_file,sep = '\t',skiprows=32)
    df['x-axis'] = pd.to_datetime(df['x-axis'])
    df['time'] = df['x-axis'] - df['x-axis'][0]
    df['dt (s)'] = df['time'].dt.total_seconds()
    return df.drop(columns=['x-axis','Unnamed: 1','Unnamed: 2','time'])
    # position = list(luna_data.columns.values)
    # luna_position = [float(i) for i in position[:-1]]

# class plot_loop:
#     def __init__(self,luna_data):
#         self.luna_data = luna_data
#         ### Get properties of strain_data
#         self.nch,self.nt = np.shape(self.luna_data)

#     def plot_loop_1(self):
